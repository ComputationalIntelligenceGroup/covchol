#' Penalized least-square estimation of Cholesky factor
#' 
#' Solve the following optimization problem
#' \deqn{\hat{L} = \arg \min_{L} ||LL^t - Sigma||_F* + ||L||_1,off}
#' 
...
#' @useDynLib covchol
#' @export
prxgradcholf <- function(X, L = diag(ncol(X)), eps =  1e-2,
                        alpha = 0.5, 
                        maxIter = 100, 
  ...
  out <- .Fortran("PRXGRDF",as.integer(ncol(X)), as.double(Cor), 
...
cholpathf <- function(X, lambdas = NULL, L =  diag(ncol(X)),
  ...
  for (i in 1:length(lambdas)){
    results[[i]] <- .Fortran("PRXGRDF",as.integer(ncol(X)), as.double(S), 
